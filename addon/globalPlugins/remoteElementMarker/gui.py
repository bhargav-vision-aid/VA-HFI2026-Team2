...
def _confirm(message_text, title="Remote Element Marker") -> bool:
    ...
    try:
        ...
        return (
            wx.MessageBox(message_text, title, wx.YES_NO | wx.ICON_QUESTION) == wx.YES
        )
...
class MarkerDialog(wx.Dialog):
    ...
    def onOk(self, evt):
        ...
        if self.shortcut:
            ...
            if self.marker_instance:
                ...
                same_doc_conflict = (
                    self.marker_instance._find_same_doc_shortcut_conflict(
    ...
    def _stopCapture(self):
        if (
            self._capturing
            and inputCore.manager
            and inputCore.manager._captureFunc == self._gestureCaptor
        ):
            inputCore.manager._captureFunc = None
        self._capturing = False
...
class MarkerManagerDialog(wx.Dialog):
    ...
    def __init__(
        ...
        sHelper.addItem(
            wx.StaticText(self, label="All saved markers by application/document:")
        )
        self.category_choice = sHelper.addLabeledControl(
            "&Category:", wx.Choice, choices=[]
        )
        self.Bind(wx.EVT_CHOICE, self.onCategoryChanged, self.category_choice)

        self.marker_list = sHelper.addLabeledControl(
            "&Markers:", wx.ListBox, choices=[]
        )
        ...
        btn_sizer.Add(
            self.edit_btn, flag=wx.RIGHT, border=gui.guiHelper.BORDER_FOR_DIALOGS
        )
        btn_sizer.Add(
            self.delete_btn, flag=wx.RIGHT, border=gui.guiHelper.BORDER_FOR_DIALOGS
        )
    ...
    def _getSelectedMarker(self):
        ...
        marker_data = (
            self.marker_instance._store.get_marker(app_key, sig_hash)
            if self.marker_instance
            else None
        )
        if not marker_data:
    ...
    def onCharHook(self, evt):
        ...
        if evt.GetKeyCode() == wx.WXK_TAB:
            ...
            if focus is self.close_btn and reverse:
                target = (
                    self.delete_btn if self.delete_btn.IsEnabled() else self.edit_btn
                )
                if target and target.IsEnabled():
                    target.SetFocus()
                    return
    ...
    def onEdit(self, evt):
        ...
        if d.ShowModal() == wx.ID_OK and self.edit_callback:
            ...
            if updated_marker:
                ...
                self.marker_list.SetString(
                    item, self._formatMarkerLabel(updated_marker)
                )
        d.Destroy()
    ...
    def onDelete(self, evt):
        ...
        if _confirm(
            f'Are you sure you want to delete the marker "{label}"?', "Confirm Deletion"
        ):
...
class MarkerPickerDialog(wx.Dialog):
    ...
    def __init__(
        ...
        self.marker_list = sHelper.addLabeledControl(
            "&Markers:", wx.ListBox, choices=choices
        )
        ...
        btn_sizer.Add(
            self.activate_btn, flag=wx.RIGHT, border=gui.guiHelper.BORDER_FOR_DIALOGS
        )
    ...
    def onEdit(self, evt):
        ...
        if d.ShowModal() == wx.ID_OK:
            ...
            shortcut_display = (
                self._formatGesture(normalized) if normalized else "No Shortcut"
            )
