...
class MarkerStore:
    ...
    def load(self) -> None:
        if os.path.exists(self._config_path):
            try:
                ...
                log.error(
                    f"Failed to load Remote Element Markers from {self._config_path}: {e}"
                )
                self._cache = {}
        else:
            self._cache = {}
    ...
    def save(self) -> None:
        try:
            ...
            log.error(
                f"Failed to save Remote Element Markers to {self._config_path}: {e}"
            )
            raise
    ...
    def set_marker(
        self, app_key: str, sig_hash: str, marker_data: Dict[str, Any]
    ) -> None:
