...
# Copyright (C) 2026 Team 2 - Optimistic Minds
# Part of Vision-Aid "Hack for Inclusion" 2026
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def __init__(self):
        ...
        self._progress_timeout_timer = None
    ...
    def _bind_saved_shortcuts(self):
        ...
        for shortcut in unique_shortcuts:
            script_name = (
                f"invokeMarkerByShortcut_{_shortcut_to_script_suffix(shortcut)}"
            )
    ...
    def _get_storage_app_key(
        self, obj, signature: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        if signature.get("backend") != "BrowseMode":
            return default_key
        target_url = (signature.get("primarySignature", {}) or {}).get(
            "url_if_web", ""
        ) or ""
        if not target_url:
            return default_key
    ...
    def _get_base_app_key(self, obj) -> str:
        ...
        app_module = (
            getattr(obj.appModule, "appModuleName", "unknown")
            if obj.appModule
            else "unknown"
        )
    ...
    def _get_markers_for_current_app(self) -> list:
        ...
        items = []
        for app_key, app_data in self._store.all_markers().items():
            if not self._app_key_matches_current_context(
                app_key, candidate_set, base_keys, doc_keys
            ):
                continue
    ...
    def _register_settings_panel(self):
        ...
        try:
            if (
                RemoteElementMarkerSettingsPanel
                not in NVDASettingsDialog.categoryClasses
            ):
    ...
    def _move_to_browse_mode_target(self, obj) -> bool:
        ...
        try:
            ...
            if callable(set_selection):
                set_selection(info)
                log.debugWarning(
                    "REM browse move: used treeInterceptor._set_selection()."
                )
            else:
    ...
    def _start_progress_beeper(
        self, operation_label: str = "Operation"
    ) -> Optional[ProgressBeeper]:
        """Start the progress beeper if beeps are enabled. Stops any running one first."""
        self._stop_progress_beeper()
        self._progress_label = operation_label
        if not self._beep_enabled:
            return None
        try:
            self._progress_beeper = ProgressBeeper()
            self._progress_beeper.start()
            self._schedule_progress_timeout_poll()
        except Exception as e:
            log.debugWarning(f"REM _start_progress_beeper error: {e}")
            self._progress_beeper = None
        return self._progress_beeper

    def _stop_progress_beeper(self, expected: Optional[ProgressBeeper] = None) -> None:
        """Stop and discard the active progress beeper."""
        pb = self._progress_beeper
        if expected is not None and pb is not expected:
            return
        self._progress_beeper = None
        self._progress_label = ""
        timer = self._progress_timeout_timer
        self._progress_timeout_timer = None
    ...
    def _schedule_progress_timeout_poll(self) -> None:
        if self._progress_timeout_timer is not None:
            return
        try:
            self._progress_timeout_timer = wx.CallLater(
                250, self._on_progress_timeout_poll
            )
        except Exception:
            self._progress_timeout_timer = None

    def _on_progress_timeout_poll(self) -> None:
        self._progress_timeout_timer = None
        pb = self._progress_beeper
        if pb is None:
            return
        if pb.is_cancelled():
            return
        self._maybe_handle_long_operation(pb, self._progress_label or "Operation")
        if self._progress_beeper is pb:
            self._schedule_progress_timeout_poll()

    def _maybe_handle_long_operation(
        self, pb: Optional[ProgressBeeper], action_name: str
    ) -> bool:
        if pb is None or not pb.consume_timeout_request():
            return True
        message = (
            f"{action_name} is taking longer than expected.\n\nDo you want to continue?"
        )
    ...
    def script_toggleAnnounceLabels(self, gesture):
        ...
        try:
            config.conf["remoteElementMarker"]["announceLabels"] = (
                self._announce_enabled
            )
        except Exception:
            pass
        if self._announce_enabled:
                    ...
                    if has_index:
                        ...
                        if live_idx >= 0 and live_idx != m_role_index:
                            ...
                            if has_context:
                                ...
                                if (
                                    m_context_before
                                    and m_context_before not in live_before
                                    and live_before not in m_context_before
                                ):
                                    ctx_ok = False
                                if (
                                    m_context_after
                                    and m_context_after not in live_after
                                    and live_after not in m_context_after
                                ):
                                    ctx_ok = False
                                if not ctx_ok:
                                    continue
    ...
    def _announce_friendly_label_for_obj(self, obj, from_timer: bool = False) -> None:
        ...
        if from_timer:
            ...
            if announcement_key == self._current_nav_marker_key:
                return
            if (now - self._last_timer_announce_time) < (
                self._nav_monitor_interval_ms / 1000.0 + 0.1
            ):
                return
    ...
    def _announce_current_navigation_label(
        self, event_obj=None, prefer_caret=False, from_timer=False
    ):
    ...
    def _register_shortcut_dispatcher(self, script_name: str, shortcut: str):
        ...
        script_func.__doc__ = (
            f"Remote Element Marker shortcut dispatcher for {shortcut}"
        )
    ...
    def _dispatch_shortcut(self, shortcut: str):
        ...
        for app_key, app_data in self._store.all_markers().items():
            if not self._app_key_matches_current_context(
                app_key, candidate_set, base_candidates, doc_candidates
            ):
                continue
    ...
    def _register_dynamic_script(
        self, script_name: str, app_key: str, sig_hash: str, friendly_name: str
    ):
    ...
    def _edit_marker_callback(
        self, app_key: str, sig_hash: str, new_name: str, new_shortcut: str
    ):
        """
        ...
        same_name_conflict = self._find_same_doc_label_conflict(
            new_name, app_key, sig_hash
        )
        if same_name_conflict:
            self._store.delete_marker(
                same_name_conflict["app_key"], same_name_conflict["sig_hash"]
            )
        ...
        self._bind_saved_shortcuts()
        log.debugWarning(
            f"REM edited marker {sig_hash}: name={new_name!r}, shortcut={normalized!r}"
        )
    ...
    def _get_browse_root(self):
        ...
        fg = api.getForegroundObject()
        log.debugWarning(
            f"REM _get_browse_root: falling back to foreground={getattr(fg, 'appName', '?')}"
        )
        return fg

    def _log_mark_capture_timing(
        self,
        start_time: float,
        source: str,
    ...
    def _beginMarkingProcess(self, obj, source):
        ...
        progress_beeper = self._start_progress_beeper(f"Capturing {source} element")

        def on_signature_ready(signature):
            self._stop_progress_beeper(progress_beeper)
            if progress_beeper is not None and progress_beeper.is_cancelled():
                ui.message("Capture cancelled.")
                return
            if not signature:
                self._log_mark_capture_timing(
                    capture_start_time, source, success=False, emptySignature=True
                )
            ...
            log.debugWarning(
                f"REM generated signature for {source} object: {signature}"
            )
        ...
        try:
            ...
            self._stop_progress_beeper(progress_beeper)
            self._log_mark_capture_timing(
                capture_start_time, source, success=False, exception=type(e).__name__
            )
    ...
    def _find_same_doc_shortcut_conflict(
        self, shortcut: str, app_key: str, sig_hash: str
    ):
        """
    ...
    def _is_shortcut_taken(self, shortcut: str, app_key: str, sig_hash: str) -> bool:
        ...
        return (
            self._find_same_doc_shortcut_conflict(shortcut, app_key, sig_hash)
            is not None
        )

    def _find_same_doc_label_conflict(
        self, friendly_name: str, app_key: str, sig_hash: str
    ):
        """
    ...
    def _save_new_marker(
        self, app_key: str, signature: Dict[str, Any], name: str, shortcut: str
    ):
        sig_hash = signature["hash"]
    ...
    def _invoke_marker(
        ...
        candidates = (
            pre_candidates if pre_candidates else self._get_app_key_candidates()
        )
        base_target = app_key.split("|doc:", 1)[0]
        base_candidates = {c.split("|doc:", 1)[0] for c in candidates}

        log.debugWarning(
            f"REM invoking marker. Candidates={candidates}, Target={app_key}"
        )

        if not ((app_key in candidates) or (base_target in base_candidates)):
            if not self._is_legacy_unstable_doc_key(app_key):
                log.warning(
                    f"App key mismatch. Target base={base_target}, Candidates={base_candidates}"
                )
                ui.message("Element not found. Application context mismatch.")
                return
        if (
            self._is_legacy_unstable_doc_key(app_key)
            and base_target not in base_candidates
        ):
            log.warning(
                f"App key mismatch. Target base={base_target}, Candidates={base_candidates}"
            )
        ...
        progress_beeper = self._start_progress_beeper(
            f"Resolving {marker_data.get('friendlyName')}"
        )
        ...
        if pre_root is not None:
            ...
            log.debugWarning(
                f"REM using pre_root: {getattr(resolve_root, 'name', '?')!r}"
            )
        else:
            resolve_root = (
        ...
        def on_resolve_done(target_obj):
            self._activate_resolved_element(target_obj, marker_data, progress_beeper)

        resolve_element(
            marker_data,
            resolve_root,
            on_resolve_done,
            progress_controller=progress_beeper,
            progress_prompt=lambda: self._maybe_handle_long_operation(
                progress_beeper,
                f"Resolving {marker_data.get('friendlyName')}",
            ),
        )

    def _activate_resolved_element(
        self,
        obj: Optional[Any],
        marker_data: Dict[str, Any],
        progress_beeper: Optional[ProgressBeeper] = None,
    ):
        # Always stop the progress beeper first, before any early return.
        was_cancelled = (
            progress_beeper.is_cancelled() if progress_beeper is not None else False
        )
        self._stop_progress_beeper(progress_beeper)

        if not obj:
            if was_cancelled:
                ui.message("Resolve cancelled.")
                return
        ...
        if isinstance(obj, bool) or not hasattr(obj, "role"):
            ...
            ui.message(
                "Element found, but the resolved target is invalid. Please re-mark the element."
            )
            if self._beep_enabled:
                beep_failure()
            return
        ...
        try:
            states = obj.states
            if (
                controlTypes.State.UNAVAILABLE in states
        ...
        if not self._activate_after_resolve and (navigated or focused):
            log.debugWarning(
                "REM activation skipped because activateAfterResolve is disabled."
            )
