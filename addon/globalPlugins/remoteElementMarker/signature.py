...
def _extract_dom_hints_from_attrs(attrs) -> Dict[str, str]:
	...
	try:
		if hasattr(attrs, "items"):
			attr_map = {str(k).strip(): _normalize_hint_value(v) for k, v in attrs.items() if _normalize_hint_value(v)}
		else:
...
def _node_matches_dom_hints(node, dom_hints: Dict[str, str]) -> bool:
	if not dom_hints:
		return False
	node_hints = _get_browse_dom_hints(node)
	if not node_hints:
		return False
...
def _compute_browse_fast_path(obj, ti) -> Dict[str, Any]:
	...
	try:
		...
		while node is not None:
			node_id = id(node)
			if node_id in seen:
				_log_browse_capture("fast_path_cycle", obj_type=type(obj).__name__, depth=len(path_steps))
				return {}
			seen.add(node_id)
			...
			try:
				...
				_log_browse_capture("fast_path_parent_error", obj_type=type(obj).__name__, depth=len(path_steps))
				return {}
			if parent is None:
				_log_browse_capture("fast_path_parent_none", obj_type=type(obj).__name__, depth=len(path_steps))
				return {}
			...
			for index, child in enumerate(_iter_children(parent)):
				if id(child) == node_id:
					...
					break
			if child_index < 0:
				_log_browse_capture("fast_path_child_not_found", obj_type=type(obj).__name__, depth=len(path_steps))
				return {}
			...
			node = parent
		_log_browse_capture("fast_path_exhausted", obj_type=type(obj).__name__, depth=len(path_steps))
	except Exception:
...
def _compute_position_hints_from_caret(obj, ti, target_role: int, target_name: str) -> Tuple[int, int]:
	"""
	...
	try:
		...
		if caret_offset < 0:
			_log_browse_capture("caret_offset_unavailable", position=caret_position, obj_type=type(obj).__name__)
			return -1, -1
			...
			if isinstance(field, textInfos.FieldCommand):
				if field.command == "controlStart":
					...
					if (
						...
						and _dom_hints_match(field_dom_hints, _get_browse_dom_hints(obj))
				...
				if field.command == "controlEnd":
					if stack:
						...
						if (
							...
							and _dom_hints_match(frame.get("dom_hints", {}), _get_browse_dom_hints(obj))
...
def _compute_position_hints(obj, ti) -> Dict[str, Any]:
	...
	"""
	_empty = {"role_index": -1, "role_ordinal": -1, "context_before": "", "context_after": ""}
	try:
		...
		target_dom_hints = _get_browse_dom_hints(obj)
		_log_browse_capture(
		...
		caret_index, caret_role_ordinal = _compute_position_hints_from_caret(obj, ti, target_role, target_name)
		if caret_index >= 0:
			_log_browse_capture("position_hints_caret_success", role_index=caret_index, role_ordinal=caret_role_ordinal)
			return {
		...
		name_counter = 0   # counts elements with (role == target_role AND name == target_name)
		...
		while stack:
			...
			if node.role == target_role:
				...
				if node_name == target_name:
					...
					if nid == id(obj):
						_log_browse_capture("position_hints_match_object_id", role_index=current_name_index, role_ordinal=current_role_ordinal)
						found_index = current_name_index
						found_role_ordinal = current_role_ordinal
						break

					# Secondary identity check: stable DOM hints.
					if _node_matches_dom_hints(node, target_dom_hints):
						_log_browse_capture("position_hints_match_dom_hints", role_index=current_name_index, role_ordinal=current_role_ordinal)
						found_index = current_name_index
						found_role_ordinal = current_role_ordinal
						break
					...
					if _node_matches_browse_path(node, target_fast_path):
						_log_browse_capture("position_hints_match_path", role_index=current_name_index, role_ordinal=current_role_ordinal)
						found_index = current_name_index
						found_role_ordinal = current_role_ordinal
						break
					...
					if target_location is not None:
						try:
							...
							if nloc and len(nloc) >= 2 and (nloc[0], nloc[1]) == target_location:
								_log_browse_capture("position_hints_match_location", role_index=current_name_index, role_ordinal=current_role_ordinal, location=target_location)
								found_index = current_name_index
								found_role_ordinal = current_role_ordinal
								break
						except Exception:
				...
				if (
					state["found_role_ordinal"] < 0
					and (
						nid == target_obj_id
						or _node_matches_dom_hints(node, target_dom_hints)
						or _node_matches_browse_path(node, target_browse_path)
					)
				):
					state["found_index"] = -1
					state["found_role_ordinal"] = current_role_ordinal
					continue
				if state["found_role_ordinal"] < 0 and target_location is not None:
					try:
						nloc = node.location
						if nloc and len(nloc) >= 2 and (nloc[0], nloc[1]) == target_location:
							state["found_index"] = -1
		...
		if found_index >= 0:
			_log_browse_capture("position_hints_tree_success", role_index=found_index, role_ordinal=found_role_ordinal)
			return {
		...
		if not timed_out:
			found_index, found_role_ordinal = _count_by_tree_order(obj, ti, target_role, target_name)
			_log_browse_capture(
...
def _count_by_tree_order(obj, ti, target_role: int, target_name: str) -> Tuple[int, int]:
	...
	try:
		...
		while stack:
			...
			if time.monotonic() - start_time > timeout_s:
				...
				_log_browse_capture("count_by_tree_timeout", visited=len(visited), name_counter=name_counter, role_counter=role_ordinal)
				break
			...
			if node.role == target_role:
				...
				if node_name == target_name:
					...
					if nid == target_id:
						_log_browse_capture("count_by_tree_match", role_index=current_name_index, role_ordinal=current_role_ordinal, visited=len(visited))
						return current_name_index, current_role_ordinal
	...
	_log_browse_capture("count_by_tree_no_match", visited=len(visited) if 'visited' in locals() else None, name_counter=name_counter if 'name_counter' in locals() else None, role_counter=role_ordinal if 'role_ordinal' in locals() else None)
	return -1, -1
...
def generate_signature_async(obj, on_done: Callable[[Dict[str, Any]], None]) -> None:
	...
	try:
		...
		if root is None:
			signature = generate_signature(obj)
			_log_signature_timing(start_time, signature.get("backend", "Unknown"), fallback=True)
			on_done(signature)
			return
		...
		if caret_index >= 0:
			_log_browse_capture("async_caret_fast_success", role_index=caret_index, role_ordinal=caret_role_ordinal)
			state = {
				"obj": obj,
				"target_role": obj.role,
				"url": url,
		...
		_log_browse_capture(
			...
			dom_hints=_get_browse_dom_hints(obj),
...
def _drive_generate_signature_browse(state: Dict[str, Any], on_done: Callable[[Dict[str, Any]], None]) -> None:
	import wx  # type: ignore
	...
	try:
		...
		for _ in range(_CHUNK_SIZE):
			...
			if not stack:
				if state["found_role_ordinal"] >= 0:
					...
					_log_browse_capture(
						...
						path_len=len(state["target_browse_path"]) if state["target_browse_path"] is not None else None,
					)
			...
			if node.role == target_role:
				...
				if node_name == target_name:
					...
					# Secondary identity check: stable DOM hints.
					if not is_target and _node_matches_dom_hints(node, target_dom_hints):
						is_target = True

					# Tertiary identity check: structural ancestry path.
					if not is_target and _node_matches_browse_path(node, target_browse_path):
						is_target = True
		...
		_log_signature_timing(state["start_time"], signature.get("backend", "Unknown"), fallback=True)
		on_done(signature)


def _finalise_browse_signature(state: Dict[str, Any], on_done: Callable[[Dict[str, Any]], None]) -> None:
